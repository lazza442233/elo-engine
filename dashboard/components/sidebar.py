...
def render_sidebar() -> dict:
    ...
    with st.sidebar.expander("How it works"):
        st.markdown(
            ...
            "account for player movement). Teams that have been in "
            "the league longer adjust more slowly — only genuine "
            "newcomers shift quickly in the early rounds.\n\n"
    ...
    with st.sidebar.expander("Technical details"):
        st.markdown(
            ...
            "a 2,000-sample Latin Hypercube search with 3-fold "
            "expanding-window walk-forward validation. The model "
            "achieves a Brier score of **0.489** on the 2025 holdout "
            "season (random guessing ≈ 0.67).\n\n"
            "- **K-factor**: 40 for a team's first 10 games, then "
            "35. Game counts carry across seasons, so only genuine "
            "newcomers use the higher initial K.\n"
            ...
            "but with diminishing returns. Expected blowouts are "
            "dampened further so mismatches don't over-inflate "
            "ratings.\n"
